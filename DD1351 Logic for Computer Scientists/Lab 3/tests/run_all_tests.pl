...
all_valid_ok([Test | Remaining]) :- 
    (verify(Test), nop();
    write(Test+' failed. The proof is valid but your program rejected it!\n')), all_valid_ok(Remaining).

all_invalid_ok([]).
all_invalid_ok([Test | Remaining]) :- 
    (\+verify(Test), nop();
    write(Test+' failed. The proof is invalid but your program accepted it!\n')), all_invalid_ok(Remaining).
